...
public class DataServiceImpl implements DataService {
	...
	public List<Coordinate> getFlightCoordinates(String flight) {
		InfluxDB influxDB = PhantomInfluxDB.connect();
		Query query = PhantomInfluxDB.query(String.format("SELECT time, latitude, longitude FROM logs WHERE flight = '%s'", flight));
		QueryResult queryResult = influxDB.query(query);
		influxDB.close();
		...
		for (List<Object> item : values) {
			String time = item.get(0).toString();
			...
			coords.add(new Coordinate(time, lat, lng));
		}
	...
	public Series getFlightData(String flight) {
		InfluxDB influxDB = PhantomInfluxDB.connect();
		Query query = PhantomInfluxDB.query(String.format("SELECT * FROM logs WHERE flight = '%s'", flight));
		QueryResult queryResult = influxDB.query(query);
		influxDB.close();
